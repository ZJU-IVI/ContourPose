...
class ResNet(nn.Module):
    def __init__(
        ...
        # without edge
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False) #将三通道改为4通道
...
class ResNet2(nn.Module):
    def __init__(
        ...
        # without edge
        self.conv1 = nn.Conv2d(64, 64, kernel_size=7, stride=2, padding=3, bias=False) #将三通道改为4通道
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
...
def resnet18_2(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet2(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls["resnet18"]))
    return model
